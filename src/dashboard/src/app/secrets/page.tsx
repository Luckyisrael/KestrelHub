...
export default function SecretsPage() {
  ...
  const { data: secrets = [], isLoading } = useQuery({
    queryKey: ['secrets'],
    queryFn: () => api<Secret[]>('/api/secrets'),
  });

  const createMut = useMutation({
    mutationFn: () => api('/api/secrets', { method: 'POST', body: JSON.stringify({ ...form, deploymentId: null }) }),
    onSuccess: () => { setShowCreate(false); setForm({ key: '', value: '', environment: 'Production' }); qc.invalidateQueries({ queryKey: ['secrets'] }); },
  });
                ...
                {secrets.map(s => (
                  <tr key={s.id} className="border-b border-[var(--border)] last:border-0">
                    ...
                    <td className="px-4 py-3">
                      <span className={s.environment === 'Production' ? 'text-red-400 text-xs' : 'text-emerald-400 text-xs'}>{s.environment}</span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1 justify-end">
                        ...
                        <button onClick={() => deleteMut.mutate(s.id)} className="p-1.5 rounded hover:bg-[var(--surface-hover)] text-red-400"><Trash2 size={15} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6 w-full max-w-md">
              ...
              <div className="space-y-3">
                <input value={form.key} onChange={e => setForm(f => ({ ...f, key: e.target.value }))} placeholder="Key" className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-3 py-2.5 text-sm text-[var(--text)] placeholder:text-[var(--text-muted)] outline-none focus:border-[var(--primary)]" />
                <input value={form.value} onChange={e => setForm(f => ({ ...f, value: e.target.value }))} placeholder="Value" type="password" className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-3 py-2.5 text-sm text-[var(--text)] placeholder:text-[var(--text-muted)] outline-none focus:border-[var(--primary)]" />
                <select value={form.environment} onChange={e => setForm(f => ({ ...f, environment: e.target.value }))} className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-3 py-2.5 text-sm text-[var(--text)] outline-none">
              ...
              <div className="flex gap-3 mt-5">
                ...
                <button onClick={() => createMut.mutate()} className="flex-1 h-10 rounded-lg text-sm font-semibold text-black" style={{ background: 'var(--primary)' }}>Save</button>
              </div>
            </div>
          </div>
        )}
